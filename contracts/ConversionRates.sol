...
contract ConversionRates is ConversionRatesInterface, VolumeImbalanceRecorder, Utils {
    ...
    function setCompactData(bytes14[] memory buy, bytes14[] memory sell, uint blockNumber, uint[] memory indices) public onlyOperator {
    ...
    function getCompactData(TRC20 token) public view returns(uint, uint, byte, byte) {
        require(tokenData[token].listed);
    ...
    function getRateByteFromCompactData(bytes32 data, TRC20 token, bool buy) internal view returns(int8) {
        uint fieldOffset = tokenData[token].compactDataFieldIndex;
        uint byteOffset;
        if (buy)
